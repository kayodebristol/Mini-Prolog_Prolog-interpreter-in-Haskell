...
notBoundedToAnother (Var a, Var b) ((Var c, Var d):ys) = if (a == c) then if (b == d) then True
																		  else False
													  	 else notBoundedToAnother (Var a, Var b) ys
notBoundedToAnother (Var a, Cons b) ((Var c, Var d):ys) = notBoundedToAnother (Var a, Cons b) ys
notBoundedToAnother (Var a, Var b) ((Var c, Cons d):ys) = notBoundedToAnother (Var a, Var b) ys
...
unifyWithHead (P query x) (P head y) = if (head == query && length(x) == length(y))
									   	   then unifyWithHead1 (P query x) (P head y)
									   else No
